...
public class OduncIslemi : Entity<Guid>
{
    ...
    public OduncIslemi(Guid id, Guid kutuphaneId, Guid kullaniciId, Guid nushaId, DateTime sonTeslimTarihi,
        DateTime? ıadeTarihi, DateTime? uzatmaTarihi, int uzatmaSayisi, OduncDurumu durumu, decimal? gecikmeCezaMiktari,
        int? gecikmeGunSayisi, string? not, Kutuphane? kutuphane, Nusha? nusha) : base(id)
    {
        KutuphaneId = kutuphaneId;
        KullaniciId = kullaniciId;
        NushaId = nushaId;
        SonTeslimTarihi = sonTeslimTarihi;
        IadeTarihi = ıadeTarihi;
        UzatmaTarihi = uzatmaTarihi;
        UzatmaSayisi = uzatmaSayisi;
        Durumu = durumu;
        GecikmeCezaMiktari = gecikmeCezaMiktari;
        GecikmeGunSayisi = gecikmeGunSayisi;
        Not = not;
        Kutuphane = kutuphane;
        Nusha = nusha;
    }
